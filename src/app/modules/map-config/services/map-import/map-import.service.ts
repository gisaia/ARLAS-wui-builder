...
export class MapImportService {
  ...
  private importLayer(
    ...
    if (layer.type === GEOMETRY_TYPE.line.toString()) {
      ...
      this.importPropertySelector(layer.paint[layer.type + '-stroke-width'], values.styleStep.strokeWidthFg, false, isAggregated, layerSource);

      values.styleStep.strokeColorFg = {}
      this.importPropertySelector(layer.paint[layer.type + '-stroke-color'], values.styleStep.strokeColorFg, false, isAggregated, layerSource);

      values.styleStep.strokeOpacityFg = {}
      this.importPropertySelector(layer.paint[layer.type + '-stroke-opacity'], values.styleStep.strokeOpacityFg, false, isAggregated, layerSource);
