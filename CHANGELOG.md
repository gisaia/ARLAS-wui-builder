# Changelog

## [v19.0.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v19.0.1) (2021-10-06)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v19.0.0...v19.0.1)

**Closed issues:**

- Render mode should be activated for 'Geometric features' only [\#604](https://github.com/gisaia/ARLAS-wui-builder/issues/604) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Set render mode to all elements by default [\#603](https://github.com/gisaia/ARLAS-wui-builder/issues/603) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- zoomMin slider can reach -3 which [\#601](https://github.com/gisaia/ARLAS-wui-builder/issues/601) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Feature/19.0.1 [\#605](https://github.com/gisaia/ARLAS-wui-builder/pull/605) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix minimal value of zoomMin control [\#602](https://github.com/gisaia/ARLAS-wui-builder/pull/602) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v19.0.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v19.0.0) (2021-09-28)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v19.0.0-beta.7...v19.0.0)

**Closed issues:**

- Stroke of polygon map style does not appear in previsualisation [\#590](https://github.com/gisaia/ARLAS-wui-builder/issues/590) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ADS](https://github.com/gisaia/ARLAS-wui-builder/labels/ADS)]
- Configurate the 'Scrollable top hits' size at 'Global configuration' section [\#580](https://github.com/gisaia/ARLAS-wui-builder/issues/580) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Add the possibility to configure the stroke of fill layer [\#579](https://github.com/gisaia/ARLAS-wui-builder/issues/579) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Configurate EO related actions to the resultlist [\#578](https://github.com/gisaia/ARLAS-wui-builder/issues/578)
- Support interaction between list elements and "scrollable top hits" layers [\#577](https://github.com/gisaia/ARLAS-wui-builder/issues/577) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Configure grid view in resultlist with a thumbnail [\#576](https://github.com/gisaia/ARLAS-wui-builder/issues/576) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Add external json nodes to the arlas configuration [\#575](https://github.com/gisaia/ARLAS-wui-builder/issues/575) [[ADS](https://github.com/gisaia/ARLAS-wui-builder/labels/ADS)]
- Apply same pagination between map and result list [\#574](https://github.com/gisaia/ARLAS-wui-builder/issues/574) [[ADS](https://github.com/gisaia/ARLAS-wui-builder/labels/ADS)]
- Synchronise data displayed on the map and the result list [\#573](https://github.com/gisaia/ARLAS-wui-builder/issues/573) [[ADS](https://github.com/gisaia/ARLAS-wui-builder/labels/ADS)]

**Merged pull requests:**

- Feature/19.0.0 [\#599](https://github.com/gisaia/ARLAS-wui-builder/pull/599) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v19.0.0-beta.7](https://github.com/gisaia/ARLAS-wui-builder/tree/v19.0.0-beta.7) (2021-09-22)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v19.0.0-beta.6...v19.0.0-beta.7)

## [v19.0.0-beta.6](https://github.com/gisaia/ARLAS-wui-builder/tree/v19.0.0-beta.6) (2021-09-20)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v18.1.1...v19.0.0-beta.6)

## [v18.1.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v18.1.1) (2021-09-14)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v18.1.0...v18.1.1)

**Merged pull requests:**

- Upgrade to toolkit 18.1.1 to fix network analytics source name [\#589](https://github.com/gisaia/ARLAS-wui-builder/pull/589) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v18.1.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v18.1.0) (2021-09-13)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v18.0.3...v18.1.0)

**Closed issues:**

- Loading a configuration with a mono-collection timeline does not work on version 18 [\#586](https://github.com/gisaia/ARLAS-wui-builder/issues/586) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[TIMELINE](https://github.com/gisaia/ARLAS-wui-builder/labels/TIMELINE)]
- Change the server url when importing a configuration file when it's not accessible [\#512](https://github.com/gisaia/ARLAS-wui-builder/issues/512) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Add a fetching precision control for network analytics layer [\#588](https://github.com/gisaia/ARLAS-wui-builder/pull/588) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v18.0.3](https://github.com/gisaia/ARLAS-wui-builder/tree/v18.0.3) (2021-08-23)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v18.0.2...v18.0.3)

**Merged pull requests:**

- Fix timeline loading with mono collection config [\#585](https://github.com/gisaia/ARLAS-wui-builder/pull/585) ([sebbousquet](https://github.com/sebbousquet))

## [v18.0.2](https://github.com/gisaia/ARLAS-wui-builder/tree/v18.0.2) (2021-08-16)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v18.0.1...v18.0.2)

**Merged pull requests:**

- Upgrade toolkit to 18.0.2 [\#584](https://github.com/gisaia/ARLAS-wui-builder/pull/584) ([sebbousquet](https://github.com/sebbousquet))

## [v18.0.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v18.0.1) (2021-08-06)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v18.0.0...v18.0.1)

**Closed issues:**

- Add the possibility to apply a timeline filter to widgets and layers of other collections  [\#557](https://github.com/gisaia/ARLAS-wui-builder/issues/557) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[TIMELINE](https://github.com/gisaia/ARLAS-wui-builder/labels/TIMELINE)] [[MULTI-COLLECTION](https://github.com/gisaia/ARLAS-wui-builder/labels/MULTI-COLLECTION)]

## [v18.0.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v18.0.0) (2021-07-12)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.2.4...v18.0.0)

**Closed issues:**

- Add the possibility to display a timeline for each collection on the same graph [\#568](https://github.com/gisaia/ARLAS-wui-builder/issues/568) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MULTI-COLLECTION](https://github.com/gisaia/ARLAS-wui-builder/labels/MULTI-COLLECTION)]

**Merged pull requests:**

- Add choice of additional collections in timeline [\#569](https://github.com/gisaia/ARLAS-wui-builder/pull/569) ([sebbousquet](https://github.com/sebbousquet))

## [v17.2.4](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.2.4) (2021-06-30)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.2.3...v17.2.4)

## [v17.2.3](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.2.3) (2021-06-30)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.2.2...v17.2.3)

## [v17.2.2](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.2.2) (2021-06-30)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.2.1...v17.2.2)

**Merged pull requests:**

- Remove basic authentication from download component [\#572](https://github.com/gisaia/ARLAS-wui-builder/pull/572) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v17.2.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.2.1) (2021-06-25)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.2.0...v17.2.1)

**Merged pull requests:**

- Fix map filters initialization [\#571](https://github.com/gisaia/ARLAS-wui-builder/pull/571) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v17.2.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.2.0) (2021-06-24)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.1.1...v17.2.0)

**Merged pull requests:**

- Upgrade to toolkit 17.2.0 [\#570](https://github.com/gisaia/ARLAS-wui-builder/pull/570) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v17.1.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.1.1) (2021-06-07)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.1.0...v17.1.1)

**Closed issues:**

- concatenate collection name to ids of all widgets [\#567](https://github.com/gisaia/ARLAS-wui-builder/issues/567) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Fix bug : concatenate collection name to ids of all widgets [\#566](https://github.com/gisaia/ARLAS-wui-builder/pull/566) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v17.1.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.1.0) (2021-06-04)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.0.1...v17.1.0)

**Closed issues:**

- Source names should include collection names [\#563](https://github.com/gisaia/ARLAS-wui-builder/issues/563) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MULTI-COLLECTION](https://github.com/gisaia/ARLAS-wui-builder/labels/MULTI-COLLECTION)]
- Metric value  precision is missing [\#561](https://github.com/gisaia/ARLAS-wui-builder/issues/561) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[WIDGET](https://github.com/gisaia/ARLAS-wui-builder/labels/WIDGET)]
- Missing collection information in contributor ID [\#559](https://github.com/gisaia/ARLAS-wui-builder/issues/559) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)] [[MULTI-COLLECTION](https://github.com/gisaia/ARLAS-wui-builder/labels/MULTI-COLLECTION)]

**Merged pull requests:**

- Upgrade to toolkit 17.1.0 [\#565](https://github.com/gisaia/ARLAS-wui-builder/pull/565) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add collection in source name to avoid source confusion [\#564](https://github.com/gisaia/ARLAS-wui-builder/pull/564) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add value precision configuration of metrics [\#562](https://github.com/gisaia/ARLAS-wui-builder/pull/562) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix \#559 [\#560](https://github.com/gisaia/ARLAS-wui-builder/pull/560) ([mbarbet](https://github.com/mbarbet))

## [v17.0.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.0.1) (2021-05-21)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.0.0...v17.0.1)

**Closed issues:**

- Add the possibility to choose which collection to explore [\#399](https://github.com/gisaia/ARLAS-wui-builder/issues/399) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Fix creation of empty dashboard [\#558](https://github.com/gisaia/ARLAS-wui-builder/pull/558) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v17.0.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.0.0) (2021-05-19)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v17.0.0-beta.0...v17.0.0)

**Closed issues:**

- Add a required step of choosing the collection from which data is fetched for layers [\#543](https://github.com/gisaia/ARLAS-wui-builder/issues/543) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)] [[MULTI-COLLECTION](https://github.com/gisaia/ARLAS-wui-builder/labels/MULTI-COLLECTION)]
- Add a required step of choosing the collection from which data is fetched for widgets [\#542](https://github.com/gisaia/ARLAS-wui-builder/issues/542) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[WIDGET](https://github.com/gisaia/ARLAS-wui-builder/labels/WIDGET)] [[MULTI-COLLECTION](https://github.com/gisaia/ARLAS-wui-builder/labels/MULTI-COLLECTION)]
- Remove filtering dashboards by collection when importing widgets or layers [\#541](https://github.com/gisaia/ARLAS-wui-builder/issues/541) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MULTI-COLLECTION](https://github.com/gisaia/ARLAS-wui-builder/labels/MULTI-COLLECTION)]

**Merged pull requests:**

- Feature/17.0.0 [\#556](https://github.com/gisaia/ARLAS-wui-builder/pull/556) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix/duplicat icon default [\#555](https://github.com/gisaia/ARLAS-wui-builder/pull/555) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Update name for i18n and tour linked files [\#554](https://github.com/gisaia/ARLAS-wui-builder/pull/554) ([mbarbet](https://github.com/mbarbet))

## [v17.0.0-beta.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v17.0.0-beta.0) (2021-05-10)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.1.3...v17.0.0-beta.0)

**Merged pull requests:**

- Feature/multi collection [\#553](https://github.com/gisaia/ARLAS-wui-builder/pull/553) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add geohash/tile agg type [\#532](https://github.com/gisaia/ARLAS-wui-builder/pull/532) ([mbarbet](https://github.com/mbarbet))

## [v16.1.3](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.1.3) (2021-04-30)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.1.2...v16.1.3)

**Merged pull requests:**

- Fix min value in map filters + upgrade to toolkit 16.1.2 [\#552](https://github.com/gisaia/ARLAS-wui-builder/pull/552) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v16.1.2](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.1.2) (2021-04-27)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.1.1...v16.1.2)

**Closed issues:**

- Range and Equal fitlers in Map are not correct [\#551](https://github.com/gisaia/ARLAS-wui-builder/issues/551) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Fix map filters for range and equal values [\#550](https://github.com/gisaia/ARLAS-wui-builder/pull/550) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v16.1.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.1.1) (2021-04-26)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.1.0...v16.1.1)

**Closed issues:**

- Duplicating a layer crashes all the layers list [\#547](https://github.com/gisaia/ARLAS-wui-builder/issues/547) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Icon of a duplicated layer is wrong [\#546](https://github.com/gisaia/ARLAS-wui-builder/issues/546) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- hover and select layers should not be available for Layer Importation [\#544](https://github.com/gisaia/ARLAS-wui-builder/issues/544) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Fix/duplicate layer [\#548](https://github.com/gisaia/ARLAS-wui-builder/pull/548) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Remove hover and select layers from Import Layer list [\#545](https://github.com/gisaia/ARLAS-wui-builder/pull/545) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v16.1.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.1.0) (2021-04-21)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.0.4...v16.1.0)

**Closed issues:**

- Add visualisation set column in the list of layers [\#536](https://github.com/gisaia/ARLAS-wui-builder/issues/536) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- When hovering/selecting a donut node, the other nodes should be transparent [\#533](https://github.com/gisaia/ARLAS-wui-builder/issues/533) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Add the possibility to "qualify" the global count unit  [\#506](https://github.com/gisaia/ARLAS-wui-builder/issues/506) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[LOOK AND FEEL](https://github.com/gisaia/ARLAS-wui-builder/labels/LOOK%20AND%20FEEL)]
- Enable the previewed layer [\#491](https://github.com/gisaia/ARLAS-wui-builder/issues/491) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Support dasharrays in line layers [\#431](https://github.com/gisaia/ARLAS-wui-builder/issues/431) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Upgrade stack to 16.1.0 [\#540](https://github.com/gisaia/ARLAS-wui-builder/pull/540) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add visualisations set in layers list [\#539](https://github.com/gisaia/ARLAS-wui-builder/pull/539) ([sebbousquet](https://github.com/sebbousquet))
- Enable the previewed layer [\#538](https://github.com/gisaia/ARLAS-wui-builder/pull/538) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add line style in layer configuration [\#537](https://github.com/gisaia/ARLAS-wui-builder/pull/537) ([sebbousquet](https://github.com/sebbousquet))
- Fix preview map [\#535](https://github.com/gisaia/ARLAS-wui-builder/pull/535) ([mbarbet](https://github.com/mbarbet))
- Fix opacity of unhovered/unselected nodes of donuts [\#534](https://github.com/gisaia/ARLAS-wui-builder/pull/534) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add option to qualify the global count unit [\#519](https://github.com/gisaia/ARLAS-wui-builder/pull/519) ([sebbousquet](https://github.com/sebbousquet))

## [v16.0.4](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.0.4) (2021-04-12)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.0.3...v16.0.4)

**Merged pull requests:**

- Fix mapcontrib for fields ending with \_count [\#531](https://github.com/gisaia/ARLAS-wui-builder/pull/531) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v16.0.3](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.0.3) (2021-04-09)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.0.2...v16.0.3)

**Closed issues:**

- Layers order inside a visualisation set is lost if one of the layers is edited [\#527](https://github.com/gisaia/ARLAS-wui-builder/issues/527) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Details of resultlist are not visible in the preview [\#525](https://github.com/gisaia/ARLAS-wui-builder/issues/525) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Remove normamisation by key for clusters [\#523](https://github.com/gisaia/ARLAS-wui-builder/issues/523) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Normalizing a field in map invalidates the form [\#522](https://github.com/gisaia/ARLAS-wui-builder/issues/522) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Sort the fields list \(in collection view\) alphabetically [\#520](https://github.com/gisaia/ARLAS-wui-builder/issues/520) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Property useColorService should be export as boolean in resultList [\#516](https://github.com/gisaia/ARLAS-wui-builder/issues/516) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Properties MargePanForLoad and MargePanForCount should be exported as number not string [\#514](https://github.com/gisaia/ARLAS-wui-builder/issues/514) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Widget layouts is lost after 2 editions of a dashboard [\#511](https://github.com/gisaia/ARLAS-wui-builder/issues/511) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Importing a layer or a widget crashes when 'dash.doc\_value.arlas' is undefined [\#510](https://github.com/gisaia/ARLAS-wui-builder/issues/510) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Sort the fields list \(in dropdown lists\) alphabetically [\#507](https://github.com/gisaia/ARLAS-wui-builder/issues/507) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Sliders positioned to 0 make the forms invalid [\#489](https://github.com/gisaia/ARLAS-wui-builder/issues/489) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)] [[layer:cluster](https://github.com/gisaia/ARLAS-wui-builder/labels/layer:cluster)]

**Merged pull requests:**

- Add missing translation [\#530](https://github.com/gisaia/ARLAS-wui-builder/pull/530) ([sebbousquet](https://github.com/sebbousquet))
- Keep layers order in visualisation set after editing them [\#529](https://github.com/gisaia/ARLAS-wui-builder/pull/529) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Display detail in resultlist preview [\#526](https://github.com/gisaia/ARLAS-wui-builder/pull/526) ([sebbousquet](https://github.com/sebbousquet))
- Fix normalisation of map layers form [\#524](https://github.com/gisaia/ARLAS-wui-builder/pull/524) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Order fields alphabetically in collection view [\#521](https://github.com/gisaia/ARLAS-wui-builder/pull/521) ([sebbousquet](https://github.com/sebbousquet))
- Check config value before acces property [\#518](https://github.com/gisaia/ARLAS-wui-builder/pull/518) ([sebbousquet](https://github.com/sebbousquet))
- Export useColorService as a boolean [\#517](https://github.com/gisaia/ARLAS-wui-builder/pull/517) ([sebbousquet](https://github.com/sebbousquet))
- Export MargePanForLoad & MargePanForCount as number [\#515](https://github.com/gisaia/ARLAS-wui-builder/pull/515) ([sebbousquet](https://github.com/sebbousquet))
- Init donut layout properly on analytics loading [\#513](https://github.com/gisaia/ARLAS-wui-builder/pull/513) ([sebbousquet](https://github.com/sebbousquet))
- Sort drop-down lists of fields alphabetically [\#509](https://github.com/gisaia/ARLAS-wui-builder/pull/509) ([sebbousquet](https://github.com/sebbousquet))
- Init interpolated fields Min and Max [\#502](https://github.com/gisaia/ARLAS-wui-builder/pull/502) ([sebbousquet](https://github.com/sebbousquet))

## [v16.0.2](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.0.2) (2021-03-31)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.0.1...v16.0.2)

**Closed issues:**

- Layer filter doesn't export good values  [\#504](https://github.com/gisaia/ARLAS-wui-builder/issues/504) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- fix infinity filter [\#503](https://github.com/gisaia/ARLAS-wui-builder/pull/503) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v16.0.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.0.1) (2021-03-31)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v16.0.0...v16.0.1)

**Closed issues:**

- Visualisation set status should be expressed by an informative icon rather than a checkbox [\#493](https://github.com/gisaia/ARLAS-wui-builder/issues/493) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Add the granularity of cluster mode in the layers list, 'Visualisation type' column [\#492](https://github.com/gisaia/ARLAS-wui-builder/issues/492) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)] [[layer:cluster](https://github.com/gisaia/ARLAS-wui-builder/labels/layer:cluster)]
- 'Taggable fields' missing translation in 'Collection' view [\#490](https://github.com/gisaia/ARLAS-wui-builder/issues/490) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Give simpler terms for fields types in 'Collection' view [\#488](https://github.com/gisaia/ARLAS-wui-builder/issues/488) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- upgrade lock files [\#505](https://github.com/gisaia/ARLAS-wui-builder/pull/505) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Improve layer and VS lisibility [\#501](https://github.com/gisaia/ARLAS-wui-builder/pull/501) ([sebbousquet](https://github.com/sebbousquet))

## [v16.0.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v16.0.0) (2021-03-29)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v15.0.2...v16.0.0)

**Closed issues:**

- Allow min 2 buckets instead of 10 in histograms [\#494](https://github.com/gisaia/ARLAS-wui-builder/issues/494) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- The Analytics Board initialisation doesn't set the layout properly [\#464](https://github.com/gisaia/ARLAS-wui-builder/issues/464) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Add the possibility to change widgets order inside a group [\#459](https://github.com/gisaia/ARLAS-wui-builder/issues/459) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Make lines caps rounded so that connections between linestrings are smooth [\#458](https://github.com/gisaia/ARLAS-wui-builder/issues/458) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Make Tab title configurable [\#441](https://github.com/gisaia/ARLAS-wui-builder/issues/441) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Rename a dashboard [\#430](https://github.com/gisaia/ARLAS-wui-builder/issues/430) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Activate color service by column [\#409](https://github.com/gisaia/ARLAS-wui-builder/issues/409) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Add the possibility to apply a filter on a configured layer [\#400](https://github.com/gisaia/ARLAS-wui-builder/issues/400) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- add the possiblity to sort the chosen network field [\#394](https://github.com/gisaia/ARLAS-wui-builder/issues/394) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)] [[layer:network-analytics](https://github.com/gisaia/ARLAS-wui-builder/labels/layer:network-analytics)]
- Add external layers: interactions with layers on the map or elements in the data table [\#388](https://github.com/gisaia/ARLAS-wui-builder/issues/388) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Duplicate a layer [\#344](https://github.com/gisaia/ARLAS-wui-builder/issues/344) [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Import an existing widget from a different dashboard [\#331](https://github.com/gisaia/ARLAS-wui-builder/issues/331) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Import an existing layer from a different dashboard [\#330](https://github.com/gisaia/ARLAS-wui-builder/issues/330) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Update lock files [\#499](https://github.com/gisaia/ARLAS-wui-builder/pull/499) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add a default style style when the column is colorized [\#498](https://github.com/gisaia/ARLAS-wui-builder/pull/498) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix drag and drop for groups [\#497](https://github.com/gisaia/ARLAS-wui-builder/pull/497) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- add the posibility to choose a nb of buckets lower than 10 [\#496](https://github.com/gisaia/ARLAS-wui-builder/pull/496) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Import widgets from other dashboards [\#495](https://github.com/gisaia/ARLAS-wui-builder/pull/495) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add external layer event [\#487](https://github.com/gisaia/ARLAS-wui-builder/pull/487) ([mbarbet](https://github.com/mbarbet))
- Add use Utc option for histogram and swimlane [\#486](https://github.com/gisaia/ARLAS-wui-builder/pull/486) ([mbarbet](https://github.com/mbarbet))
- Add rename in action list [\#485](https://github.com/gisaia/ARLAS-wui-builder/pull/485) ([mbarbet](https://github.com/mbarbet))
- Add ordering for feature metric [\#484](https://github.com/gisaia/ARLAS-wui-builder/pull/484) ([mbarbet](https://github.com/mbarbet))
- Import a layer from other dashboard [\#483](https://github.com/gisaia/ARLAS-wui-builder/pull/483) ([sebbousquet](https://github.com/sebbousquet))
- Add edit name button [\#482](https://github.com/gisaia/ARLAS-wui-builder/pull/482) ([mbarbet](https://github.com/mbarbet))
- Add config for tab title [\#481](https://github.com/gisaia/ARLAS-wui-builder/pull/481) ([mbarbet](https://github.com/mbarbet))
- Smooth line on the map [\#480](https://github.com/gisaia/ARLAS-wui-builder/pull/480) ([mbarbet](https://github.com/mbarbet))
- Add color by column [\#479](https://github.com/gisaia/ARLAS-wui-builder/pull/479) ([mbarbet](https://github.com/mbarbet))
- Fix Generated Color [\#478](https://github.com/gisaia/ARLAS-wui-builder/pull/478) ([mbarbet](https://github.com/mbarbet))
- Add provided color for powerbars [\#477](https://github.com/gisaia/ARLAS-wui-builder/pull/477) ([mbarbet](https://github.com/mbarbet))
- Add mapbox filters to the layers [\#476](https://github.com/gisaia/ARLAS-wui-builder/pull/476) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix layer duplication [\#475](https://github.com/gisaia/ARLAS-wui-builder/pull/475) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Deal with duplicate name on creation [\#474](https://github.com/gisaia/ARLAS-wui-builder/pull/474) ([mbarbet](https://github.com/mbarbet))
- Drag widget in group [\#473](https://github.com/gisaia/ARLAS-wui-builder/pull/473) ([mbarbet](https://github.com/mbarbet))
- Add the possibility to duplicate layer [\#472](https://github.com/gisaia/ARLAS-wui-builder/pull/472) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Rename Dashboard [\#471](https://github.com/gisaia/ARLAS-wui-builder/pull/471) ([mbarbet](https://github.com/mbarbet))
- Fix layout initialisation of AB [\#469](https://github.com/gisaia/ARLAS-wui-builder/pull/469) ([sebbousquet](https://github.com/sebbousquet))

## [v15.0.2](https://github.com/gisaia/ARLAS-wui-builder/tree/v15.0.2) (2021-02-25)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v15.0.1...v15.0.2)

**Closed issues:**

- Resultlist details of an item are displayed in red in preview [\#466](https://github.com/gisaia/ARLAS-wui-builder/issues/466) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]

**Merged pull requests:**

- Upgrade to toolkit 15.0.2 [\#468](https://github.com/gisaia/ARLAS-wui-builder/pull/468) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add missing style for detailed item in resultlist [\#467](https://github.com/gisaia/ARLAS-wui-builder/pull/467) ([sebbousquet](https://github.com/sebbousquet))

## [v15.0.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v15.0.1) (2021-02-23)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v15.0.0...v15.0.1)

**Closed issues:**

- Add a new Granularity for cluster mode [\#333](https://github.com/gisaia/ARLAS-wui-builder/issues/333) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Upgrade toolkit to 15.0.1 [\#465](https://github.com/gisaia/ARLAS-wui-builder/pull/465) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v15.0.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v15.0.0) (2021-02-18)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v15.0.0-beta.0...v15.0.0)

**Closed issues:**

- Creating 2 layers with the same name creates display issues on the visualisation sets [\#460](https://github.com/gisaia/ARLAS-wui-builder/issues/460)
- Change 'Timeline' icon in the left menu, using a time notion [\#452](https://github.com/gisaia/ARLAS-wui-builder/issues/452) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Enhance display of layers list in Visualisation set form [\#451](https://github.com/gisaia/ARLAS-wui-builder/issues/451) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Fix translation for stoke color, opacity, width [\#450](https://github.com/gisaia/ARLAS-wui-builder/issues/450) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Data table previsualisation doesn't have a fixed size \(no scrolling for large tables\) [\#434](https://github.com/gisaia/ARLAS-wui-builder/issues/434) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Set Data table size to 100 instead of 0 currently [\#433](https://github.com/gisaia/ARLAS-wui-builder/issues/433) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Add description about authentication to download data [\#429](https://github.com/gisaia/ARLAS-wui-builder/issues/429) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[SIDE](https://github.com/gisaia/ARLAS-wui-builder/labels/SIDE)]
- Add min/max features columns in Layers table [\#408](https://github.com/gisaia/ARLAS-wui-builder/issues/408) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Add the possibility to define a metric in Powerbars [\#406](https://github.com/gisaia/ARLAS-wui-builder/issues/406) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- widgets layouts is not applied when view it in ARLAS-wui [\#367](https://github.com/gisaia/ARLAS-wui-builder/issues/367) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[WIDGET](https://github.com/gisaia/ARLAS-wui-builder/labels/WIDGET)]

**Merged pull requests:**

- Feature/donut container width [\#463](https://github.com/gisaia/ARLAS-wui-builder/pull/463) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add SuperFinest granularity [\#462](https://github.com/gisaia/ARLAS-wui-builder/pull/462) ([mbarbet](https://github.com/mbarbet))
- Allow to create seveeral layers with the same name [\#461](https://github.com/gisaia/ARLAS-wui-builder/pull/461) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix max\_age\_cache [\#457](https://github.com/gisaia/ARLAS-wui-builder/pull/457) ([mbarbet](https://github.com/mbarbet))
- Add min/max feature column in layer list [\#456](https://github.com/gisaia/ARLAS-wui-builder/pull/456) ([sebbousquet](https://github.com/sebbousquet))
- Add metrics to powerbars [\#455](https://github.com/gisaia/ARLAS-wui-builder/pull/455) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix/description [\#454](https://github.com/gisaia/ARLAS-wui-builder/pull/454) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add some graphical imrprovment [\#453](https://github.com/gisaia/ARLAS-wui-builder/pull/453) ([sebbousquet](https://github.com/sebbousquet))
- Fix resultlist display [\#449](https://github.com/gisaia/ARLAS-wui-builder/pull/449) ([sebbousquet](https://github.com/sebbousquet))
- Properly clear ref to elem [\#448](https://github.com/gisaia/ARLAS-wui-builder/pull/448) ([sebbousquet](https://github.com/sebbousquet))

## [v15.0.0-beta.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v15.0.0-beta.0) (2021-02-04)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.3.1...v15.0.0-beta.0)

## [v14.3.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.3.1) (2021-02-03)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.3.0...v14.3.1)

**Closed issues:**

- Color picker is badly positioned if the page is scrolled [\#444](https://github.com/gisaia/ARLAS-wui-builder/issues/444) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Update toolkit to v14.3.1 [\#446](https://github.com/gisaia/ARLAS-wui-builder/pull/446) ([sebbousquet](https://github.com/sebbousquet))
- Fix color picker position on scrolled page [\#445](https://github.com/gisaia/ARLAS-wui-builder/pull/445) ([sebbousquet](https://github.com/sebbousquet))

## [v14.3.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.3.0) (2021-02-01)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.2.3...v14.3.0)

**Closed issues:**

- Allow 'localhost' in Tagger urls [\#439](https://github.com/gisaia/ARLAS-wui-builder/issues/439) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Columns & Details layouts don't have the same size [\#435](https://github.com/gisaia/ARLAS-wui-builder/issues/435) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Editing a dashboard get stuck in "Landing page" [\#432](https://github.com/gisaia/ARLAS-wui-builder/issues/432) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Activate CSV export for widgets [\#428](https://github.com/gisaia/ARLAS-wui-builder/issues/428) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[WIDGET](https://github.com/gisaia/ARLAS-wui-builder/labels/WIDGET)]

**Merged pull requests:**

- Upgrade toolkit to 14.3.0 [\#443](https://github.com/gisaia/ARLAS-wui-builder/pull/443) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Align analytics board css to match arlas-wui's [\#442](https://github.com/gisaia/ARLAS-wui-builder/pull/442) ([sebbousquet](https://github.com/sebbousquet))
- Allow host and port in tagger url [\#440](https://github.com/gisaia/ARLAS-wui-builder/pull/440) ([sebbousquet](https://github.com/sebbousquet))
- Fix manual property color init [\#438](https://github.com/gisaia/ARLAS-wui-builder/pull/438) ([sebbousquet](https://github.com/sebbousquet))
- Add export CSV button to donut,histogram,powerbars [\#437](https://github.com/gisaia/ARLAS-wui-builder/pull/437) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.2.3](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.2.3) (2021-01-25)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.2.2...v14.2.3)

**Closed issues:**

- Interpolation of negative values to color palette is not working [\#425](https://github.com/gisaia/ARLAS-wui-builder/issues/425) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Interpolation of color to values between 0 - 1 is not working [\#407](https://github.com/gisaia/ARLAS-wui-builder/issues/407) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Fix geometry type value on load [\#427](https://github.com/gisaia/ARLAS-wui-builder/pull/427) ([sebbousquet](https://github.com/sebbousquet))
- Fix interpolation for negative and float number [\#426](https://github.com/gisaia/ARLAS-wui-builder/pull/426) ([sebbousquet](https://github.com/sebbousquet))

## [v14.2.2](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.2.2) (2021-01-19)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.2.1...v14.2.2)

**Merged pull requests:**

- Feature/14.2.2 [\#424](https://github.com/gisaia/ARLAS-wui-builder/pull/424) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.2.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.2.1) (2021-01-18)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.2.0...v14.2.1)

**Merged pull requests:**

- Add permissions variable [\#423](https://github.com/gisaia/ARLAS-wui-builder/pull/423) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.2.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.2.0) (2021-01-18)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.1.2...v14.2.0)

**Closed issues:**

- Add the possibility to interpolate the opacity to the value of a given field [\#398](https://github.com/gisaia/ARLAS-wui-builder/issues/398) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Enhance Geomtery type according to the chosen geometry [\#396](https://github.com/gisaia/ARLAS-wui-builder/issues/396) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Add the possibility to configure the circle stroke [\#343](https://github.com/gisaia/ARLAS-wui-builder/issues/343) [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Add a dedicated page for collection description [\#329](https://github.com/gisaia/ARLAS-wui-builder/issues/329) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Manage basemap url in map configuration [\#308](https://github.com/gisaia/ARLAS-wui-builder/issues/308) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]

**Merged pull requests:**

- Upgrade to toolkit 14.2.0 [\#422](https://github.com/gisaia/ARLAS-wui-builder/pull/422) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Init list of basemaps with a default one, if none defined [\#421](https://github.com/gisaia/ARLAS-wui-builder/pull/421) ([sebbousquet](https://github.com/sebbousquet))
- Add basemaps env variable in start.sh [\#420](https://github.com/gisaia/ARLAS-wui-builder/pull/420) ([sebbousquet](https://github.com/sebbousquet))
- Init default basemap on dashboard creation [\#419](https://github.com/gisaia/ARLAS-wui-builder/pull/419) ([sebbousquet](https://github.com/sebbousquet))
- Add basemap manager [\#418](https://github.com/gisaia/ARLAS-wui-builder/pull/418) ([sebbousquet](https://github.com/sebbousquet))
- Feature/collection description [\#417](https://github.com/gisaia/ARLAS-wui-builder/pull/417) ([sebbousquet](https://github.com/sebbousquet))
- Set geometry type according to chosen geometry [\#416](https://github.com/gisaia/ARLAS-wui-builder/pull/416) ([sebbousquet](https://github.com/sebbousquet))
- Add labels in changelog generation [\#415](https://github.com/gisaia/ARLAS-wui-builder/pull/415) ([sebbousquet](https://github.com/sebbousquet))
- Add circle stroke configuration [\#414](https://github.com/gisaia/ARLAS-wui-builder/pull/414) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add the possibility to interpolate the opacity to the value of a given field [\#413](https://github.com/gisaia/ARLAS-wui-builder/pull/413) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.1.2](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.1.2) (2020-12-21)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.1.1...v14.1.2)

**Closed issues:**

- Static links environment variable not declared in start.sh [\#411](https://github.com/gisaia/ARLAS-wui-builder/issues/411) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Add static links env variable in start.sh [\#412](https://github.com/gisaia/ARLAS-wui-builder/pull/412) ([sebbousquet](https://github.com/sebbousquet))

## [v14.1.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.1.1) (2020-12-18)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.1.0...v14.1.1)

**Closed issues:**

- Fix the minimum radius value [\#397](https://github.com/gisaia/ARLAS-wui-builder/issues/397) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Disable the grid aspect [\#395](https://github.com/gisaia/ARLAS-wui-builder/issues/395) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)] [[layer:network-analytics](https://github.com/gisaia/ARLAS-wui-builder/labels/layer:network-analytics)]
- Adapt default values of look & feel [\#393](https://github.com/gisaia/ARLAS-wui-builder/issues/393) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[LOOK AND FEEL](https://github.com/gisaia/ARLAS-wui-builder/labels/LOOK%20AND%20FEEL)]
- Lines : make default Width to fix with a given value [\#392](https://github.com/gisaia/ARLAS-wui-builder/issues/392) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Add the possibility to activate coordinates on the map [\#390](https://github.com/gisaia/ARLAS-wui-builder/issues/390) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Change the order of layers inside visualisation sets [\#361](https://github.com/gisaia/ARLAS-wui-builder/issues/361) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Change the order of visualisation sets [\#360](https://github.com/gisaia/ARLAS-wui-builder/issues/360) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- Choose circle size smaller than 5 [\#349](https://github.com/gisaia/ARLAS-wui-builder/issues/349) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- DATA TABLE : Width of the tabs Data/render is not adapted \(too small\) \(resultlist\) [\#334](https://github.com/gisaia/ARLAS-wui-builder/issues/334) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Traduction / Description [\#318](https://github.com/gisaia/ARLAS-wui-builder/issues/318) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Show error message in popup when something went wrong [\#288](https://github.com/gisaia/ARLAS-wui-builder/issues/288) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Add static links section [\#410](https://github.com/gisaia/ARLAS-wui-builder/pull/410) ([sebbousquet](https://github.com/sebbousquet))
- Disable granularity for network analytics [\#405](https://github.com/gisaia/ARLAS-wui-builder/pull/405) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix/black dots [\#404](https://github.com/gisaia/ARLAS-wui-builder/pull/404) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix size of resultlist form [\#403](https://github.com/gisaia/ARLAS-wui-builder/pull/403) ([sebbousquet](https://github.com/sebbousquet))
- Add some default values [\#402](https://github.com/gisaia/ARLAS-wui-builder/pull/402) ([sebbousquet](https://github.com/sebbousquet))
- Add display coordinates options [\#401](https://github.com/gisaia/ARLAS-wui-builder/pull/401) ([mbarbet](https://github.com/mbarbet))
- Add the possibility to order layers/visualisations [\#389](https://github.com/gisaia/ARLAS-wui-builder/pull/389) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.1.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.1.0) (2020-12-07)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.0.1...v14.1.0)

**Closed issues:**

- Don't reset writers/readers access on dashboard update [\#385](https://github.com/gisaia/ARLAS-wui-builder/issues/385) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Replace Configuration occurences by Dashboard [\#384](https://github.com/gisaia/ARLAS-wui-builder/issues/384)
- Edit tab's title directly [\#381](https://github.com/gisaia/ARLAS-wui-builder/issues/381) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- Make layers and visualisation sets lists optional [\#379](https://github.com/gisaia/ARLAS-wui-builder/issues/379) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- \[HISTOGRAM\] Set number of buckets at 50 [\#369](https://github.com/gisaia/ARLAS-wui-builder/issues/369) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- \[DONUT\] removing a field doesn't work [\#368](https://github.com/gisaia/ARLAS-wui-builder/issues/368) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- \[DONUT\] set donut size to 10 by default [\#366](https://github.com/gisaia/ARLAS-wui-builder/issues/366) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[ANALYTICS](https://github.com/gisaia/ARLAS-wui-builder/labels/ANALYTICS)]
- \[HEATMAP\] the opacity of heatmap layers doesn't work [\#365](https://github.com/gisaia/ARLAS-wui-builder/issues/365) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- \[LAYER\]: switch "visibility" and "style" steps order [\#364](https://github.com/gisaia/ARLAS-wui-builder/issues/364) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- \[LAYER\]: Hide collection step as we have always one [\#363](https://github.com/gisaia/ARLAS-wui-builder/issues/363) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)]
- "Ajouter une couche de données" and "Ajouter un espace de visualisation" text is not wrapped inside the button [\#362](https://github.com/gisaia/ARLAS-wui-builder/issues/362) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Enhance the default 'look and feel' elements [\#359](https://github.com/gisaia/ARLAS-wui-builder/issues/359) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[LOOK AND FEEL](https://github.com/gisaia/ARLAS-wui-builder/labels/LOOK%20AND%20FEEL)]
- Set default search placeholder [\#358](https://github.com/gisaia/ARLAS-wui-builder/issues/358) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[SEARCH](https://github.com/gisaia/ARLAS-wui-builder/labels/SEARCH)]
- Minimal zoom for cluster & Network analytics  [\#357](https://github.com/gisaia/ARLAS-wui-builder/issues/357) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)] [[MAP](https://github.com/gisaia/ARLAS-wui-builder/labels/MAP)] [[layer:network-analytics](https://github.com/gisaia/ARLAS-wui-builder/labels/layer:network-analytics)] [[layer:cluster](https://github.com/gisaia/ARLAS-wui-builder/labels/layer:cluster)]
- Enhance default configuration of the timeline [\#356](https://github.com/gisaia/ARLAS-wui-builder/issues/356) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] [[TIMELINE](https://github.com/gisaia/ARLAS-wui-builder/labels/TIMELINE)]
- Add a spinner after loading the collections list [\#355](https://github.com/gisaia/ARLAS-wui-builder/issues/355) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Interpolation of width & radius to a field doesn't work [\#354](https://github.com/gisaia/ARLAS-wui-builder/issues/354) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Renaming a map layer causes crash [\#350](https://github.com/gisaia/ARLAS-wui-builder/issues/350) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- MAP : add filter to a map layer [\#347](https://github.com/gisaia/ARLAS-wui-builder/issues/347) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- MAP: delete a visualisation set [\#345](https://github.com/gisaia/ARLAS-wui-builder/issues/345)
- Add missing english description [\#335](https://github.com/gisaia/ARLAS-wui-builder/issues/335) [[type:documentation](https://github.com/gisaia/ARLAS-wui-builder/labels/type:documentation)] [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Manage colors globally [\#213](https://github.com/gisaia/ARLAS-wui-builder/issues/213) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Replace 'Configuration' by 'Dashboard' [\#387](https://github.com/gisaia/ARLAS-wui-builder/pull/387) ([sebbousquet](https://github.com/sebbousquet))
- Add writers and readers in update dashboard [\#386](https://github.com/gisaia/ARLAS-wui-builder/pull/386) ([mbarbet](https://github.com/mbarbet))
- Fix Manual color mode [\#383](https://github.com/gisaia/ARLAS-wui-builder/pull/383) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Edit tab text on focus [\#382](https://github.com/gisaia/ARLAS-wui-builder/pull/382) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Make layers and visualisation lists optional [\#380](https://github.com/gisaia/ARLAS-wui-builder/pull/380) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Display properly button 'Add visualisation set' [\#378](https://github.com/gisaia/ARLAS-wui-builder/pull/378) ([sebbousquet](https://github.com/sebbousquet))
- Apply heatmap opacity [\#377](https://github.com/gisaia/ARLAS-wui-builder/pull/377) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Delete a visualisation set [\#376](https://github.com/gisaia/ARLAS-wui-builder/pull/376) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Init timeline aggregation with collection timepath [\#375](https://github.com/gisaia/ARLAS-wui-builder/pull/375) ([sebbousquet](https://github.com/sebbousquet))
- Fix donut and metric fields management [\#374](https://github.com/gisaia/ARLAS-wui-builder/pull/374) ([sebbousquet](https://github.com/sebbousquet))
- Hide collection step [\#373](https://github.com/gisaia/ARLAS-wui-builder/pull/373) ([sebbousquet](https://github.com/sebbousquet))
- Add a spinner after loading the collection list [\#372](https://github.com/gisaia/ARLAS-wui-builder/pull/372) ([mbarbet](https://github.com/mbarbet))
- Set some default values [\#371](https://github.com/gisaia/ARLAS-wui-builder/pull/371) ([sebbousquet](https://github.com/sebbousquet))
- Remove old layer name after renaming the layer [\#353](https://github.com/gisaia/ARLAS-wui-builder/pull/353) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix interpolation of width and radius [\#348](https://github.com/gisaia/ARLAS-wui-builder/pull/348) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add map filter when interpolating color/radius/width to avoid black style [\#346](https://github.com/gisaia/ARLAS-wui-builder/pull/346) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.0.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.0.1) (2020-11-13)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.0.0...v14.0.1)

**Closed issues:**

- Fix Look and feel attributes format [\#340](https://github.com/gisaia/ARLAS-wui-builder/issues/340) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Upgrade to toolkit 14.0.1 [\#342](https://github.com/gisaia/ARLAS-wui-builder/pull/342) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix look&feel options format: [\#341](https://github.com/gisaia/ARLAS-wui-builder/pull/341) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.0.0](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.0.0) (2020-11-09)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.0.0-rc.8...v14.0.0)

**Closed issues:**

- Fix width of dropdown list to see the whole field path [\#327](https://github.com/gisaia/ARLAS-wui-builder/issues/327) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Disable fields in dropdown lists when fields are not indexed [\#326](https://github.com/gisaia/ARLAS-wui-builder/issues/326) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Manage arlas wui name in builder [\#317](https://github.com/gisaia/ARLAS-wui-builder/issues/317) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Deal with arlas taggable field [\#219](https://github.com/gisaia/ARLAS-wui-builder/issues/219) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Update toolkit to v14.0.0 [\#339](https://github.com/gisaia/ARLAS-wui-builder/pull/339) ([sebbousquet](https://github.com/sebbousquet))
- Fix aggregation type in histogram & swimlane when choosing intervals [\#338](https://github.com/gisaia/ARLAS-wui-builder/pull/338) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix provided color bug [\#337](https://github.com/gisaia/ARLAS-wui-builder/pull/337) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Enhance translations [\#332](https://github.com/gisaia/ARLAS-wui-builder/pull/332) ([sebbousquet](https://github.com/sebbousquet))
- disable not-indexed fields + width of mat-select [\#328](https://github.com/gisaia/ARLAS-wui-builder/pull/328) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix creation of first visualisation set [\#325](https://github.com/gisaia/ARLAS-wui-builder/pull/325) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix flatting taggable fields [\#324](https://github.com/gisaia/ARLAS-wui-builder/pull/324) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Support app name and unit as unmanaged fields [\#322](https://github.com/gisaia/ARLAS-wui-builder/pull/322) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add some minor display improvements [\#320](https://github.com/gisaia/ARLAS-wui-builder/pull/320) ([sebbousquet](https://github.com/sebbousquet))
- Display palette previex only when property type is color [\#319](https://github.com/gisaia/ARLAS-wui-builder/pull/319) ([sebbousquet](https://github.com/sebbousquet))

## [v14.0.0-rc.8](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.0.0-rc.8) (2020-10-22)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.0.0-rc.7...v14.0.0-rc.8)

**Closed issues:**

- No error message should be displayed by default [\#302](https://github.com/gisaia/ARLAS-wui-builder/issues/302) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Remove error message on widget creation [\#316](https://github.com/gisaia/ARLAS-wui-builder/pull/316) ([sebbousquet](https://github.com/sebbousquet))

## [v14.0.0-rc.7](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.0.0-rc.7) (2020-10-20)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.0.0-rc.6...v14.0.0-rc.7)

**Merged pull requests:**

- Enhance forms [\#315](https://github.com/gisaia/ARLAS-wui-builder/pull/315) ([sebbousquet](https://github.com/sebbousquet))
- Use keyToColors list in the preview of analytics [\#314](https://github.com/gisaia/ARLAS-wui-builder/pull/314) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Select at least one visualisation set when creating layers [\#313](https://github.com/gisaia/ARLAS-wui-builder/pull/313) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.0.0-rc.6](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.0.0-rc.6) (2020-10-16)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.0.0-rc.5...v14.0.0-rc.6)

**Closed issues:**

- Use basic button style in analytics section [\#305](https://github.com/gisaia/ARLAS-wui-builder/issues/305) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Remove the possibility to open the left menu [\#304](https://github.com/gisaia/ARLAS-wui-builder/issues/304) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Remove or lighten the background-color of description section [\#303](https://github.com/gisaia/ARLAS-wui-builder/issues/303) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Define a primary version of Button [\#301](https://github.com/gisaia/ARLAS-wui-builder/issues/301) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Homogenise color from sliders and slide toggles [\#300](https://github.com/gisaia/ARLAS-wui-builder/issues/300) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Add some default value [\#285](https://github.com/gisaia/ARLAS-wui-builder/issues/285) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Add possibility to regroup linked fields in block [\#223](https://github.com/gisaia/ARLAS-wui-builder/issues/223) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Do not restrict the number of widgets per group [\#184](https://github.com/gisaia/ARLAS-wui-builder/issues/184) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Enhance aggregation text + some default values [\#312](https://github.com/gisaia/ARLAS-wui-builder/pull/312) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Lighten form fields render [\#311](https://github.com/gisaia/ARLAS-wui-builder/pull/311) ([sebbousquet](https://github.com/sebbousquet))
- Add missing default values [\#310](https://github.com/gisaia/ARLAS-wui-builder/pull/310) ([sebbousquet](https://github.com/sebbousquet))
- Update logo and remove possibility to open the left menu [\#309](https://github.com/gisaia/ARLAS-wui-builder/pull/309) ([sebbousquet](https://github.com/sebbousquet))
- Lot of graphics improvement [\#307](https://github.com/gisaia/ARLAS-wui-builder/pull/307) ([sebbousquet](https://github.com/sebbousquet))
- Fix cssCustomizedCss for swimlanes [\#306](https://github.com/gisaia/ARLAS-wui-builder/pull/306) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.0.0-rc.5](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.0.0-rc.5) (2020-10-14)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v14.0.0-rc.1...v14.0.0-rc.5)

**Closed issues:**

- Deleted layers are kept in visaulisationsSets [\#296](https://github.com/gisaia/ARLAS-wui-builder/issues/296) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Fix duplicated ids of contributors [\#294](https://github.com/gisaia/ARLAS-wui-builder/issues/294) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Add configuration object validation with the schema [\#293](https://github.com/gisaia/ARLAS-wui-builder/issues/293) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- InitCentre form stores values as string instead of number [\#289](https://github.com/gisaia/ARLAS-wui-builder/issues/289)
- The saved configuration does not appears on the main list. [\#284](https://github.com/gisaia/ARLAS-wui-builder/issues/284) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Error message in console on the start of the app [\#283](https://github.com/gisaia/ARLAS-wui-builder/issues/283) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- App review [\#244](https://github.com/gisaia/ARLAS-wui-builder/issues/244)

**Merged pull requests:**

- Fix/interpolate colors [\#299](https://github.com/gisaia/ARLAS-wui-builder/pull/299) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Validate configuration against json schema before save [\#298](https://github.com/gisaia/ARLAS-wui-builder/pull/298) ([sebbousquet](https://github.com/sebbousquet))
- Fix manual color [\#297](https://github.com/gisaia/ARLAS-wui-builder/pull/297) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix map property selector [\#295](https://github.com/gisaia/ARLAS-wui-builder/pull/295) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix layer source metrics + duplicated contributors [\#292](https://github.com/gisaia/ARLAS-wui-builder/pull/292) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Inverse long/lat in initcenter [\#291](https://github.com/gisaia/ARLAS-wui-builder/pull/291) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix type of initCenter [\#290](https://github.com/gisaia/ARLAS-wui-builder/pull/290) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix/load [\#287](https://github.com/gisaia/ARLAS-wui-builder/pull/287) ([sebbousquet](https://github.com/sebbousquet))
- Fix/authent [\#286](https://github.com/gisaia/ARLAS-wui-builder/pull/286) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v14.0.0-rc.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v14.0.0-rc.1) (2020-09-29)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v0.0.6-beta.1...v14.0.0-rc.1)

**Closed issues:**

- Histogram / Timeline configuration : interval value is a string instead of a number [\#276](https://github.com/gisaia/ARLAS-wui-builder/issues/276) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- After saving a new configuration, user cannot update it until the app was reloaded [\#275](https://github.com/gisaia/ARLAS-wui-builder/issues/275) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Deal with 401/403 errors when we save a config with persistence [\#248](https://github.com/gisaia/ARLAS-wui-builder/issues/248) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Open modal with login again message after 403 or silent-refresh error [\#247](https://github.com/gisaia/ARLAS-wui-builder/issues/247) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- App crash in analytics group creation when a widget is defined [\#246](https://github.com/gisaia/ARLAS-wui-builder/issues/246) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- TranslateHttpLoader use hardcoding /assets/i18n and doest not work with custum app base href [\#241](https://github.com/gisaia/ARLAS-wui-builder/issues/241) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Enrich header to access a protected ARLAS-server [\#237](https://github.com/gisaia/ARLAS-wui-builder/issues/237) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Manage Visualisation sets [\#230](https://github.com/gisaia/ARLAS-wui-builder/issues/230) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Add 'Medium' granularity for cluster and feature-metric layers [\#224](https://github.com/gisaia/ARLAS-wui-builder/issues/224) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Fix generation of contributor names  [\#174](https://github.com/gisaia/ARLAS-wui-builder/issues/174) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Add max\_old\_space opion in build [\#282](https://github.com/gisaia/ARLAS-wui-builder/pull/282) ([sebbousquet](https://github.com/sebbousquet))
- Update toolkit [\#281](https://github.com/gisaia/ARLAS-wui-builder/pull/281) ([sebbousquet](https://github.com/sebbousquet))
- Feature/current config [\#280](https://github.com/gisaia/ARLAS-wui-builder/pull/280) ([sebbousquet](https://github.com/sebbousquet))
- Fix multiple descibe calls, that blocks the application from loading at config import [\#279](https://github.com/gisaia/ARLAS-wui-builder/pull/279) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Export interval value as number [\#278](https://github.com/gisaia/ARLAS-wui-builder/pull/278) ([sebbousquet](https://github.com/sebbousquet))
- Add edit widget menu [\#277](https://github.com/gisaia/ARLAS-wui-builder/pull/277) ([sebbousquet](https://github.com/sebbousquet))
- Fix interpolation of radius [\#274](https://github.com/gisaia/ARLAS-wui-builder/pull/274) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Review analytics configuration interface [\#273](https://github.com/gisaia/ARLAS-wui-builder/pull/273) ([sebbousquet](https://github.com/sebbousquet))
- Add button to manager widget position in a group [\#272](https://github.com/gisaia/ARLAS-wui-builder/pull/272) ([sebbousquet](https://github.com/sebbousquet))
- Add titles to map global configuration [\#271](https://github.com/gisaia/ARLAS-wui-builder/pull/271) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Set default values [\#270](https://github.com/gisaia/ARLAS-wui-builder/pull/270) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add the possibility to delete a widget [\#269](https://github.com/gisaia/ARLAS-wui-builder/pull/269) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Group metric collect&field controls in the same block + translations [\#268](https://github.com/gisaia/ARLAS-wui-builder/pull/268) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Enhance the histogram edition form [\#267](https://github.com/gisaia/ARLAS-wui-builder/pull/267) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Make widget selection more user friendly [\#266](https://github.com/gisaia/ARLAS-wui-builder/pull/266) ([sebbousquet](https://github.com/sebbousquet))
- Differenciate keys [\#265](https://github.com/gisaia/ARLAS-wui-builder/pull/265) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Feature/bucket type [\#264](https://github.com/gisaia/ARLAS-wui-builder/pull/264) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix powerbars title [\#263](https://github.com/gisaia/ARLAS-wui-builder/pull/263) ([sebbousquet](https://github.com/sebbousquet))
- Distinguish between timeline and histograms markers [\#262](https://github.com/gisaia/ARLAS-wui-builder/pull/262) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Use count as metric in widgets config [\#261](https://github.com/gisaia/ARLAS-wui-builder/pull/261) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add new unmanaged fields [\#260](https://github.com/gisaia/ARLAS-wui-builder/pull/260) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Integrate new translation [\#259](https://github.com/gisaia/ARLAS-wui-builder/pull/259) ([sebbousquet](https://github.com/sebbousquet))
- Fix query-op value + add hidden idFieldName [\#258](https://github.com/gisaia/ARLAS-wui-builder/pull/258) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Save configuration + Preview with Visualisation sets [\#257](https://github.com/gisaia/ARLAS-wui-builder/pull/257) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix some CamelCase bugs [\#256](https://github.com/gisaia/ARLAS-wui-builder/pull/256) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add the possibility to configure visualisation sets [\#255](https://github.com/gisaia/ARLAS-wui-builder/pull/255) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v0.0.6-beta.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v0.0.6-beta.1) (2020-08-25)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v0.0.5...v0.0.6-beta.1)

**Closed issues:**

- Store conf id to update with unique key LOCALSTORAGE\_CONFIG\_ID\_KEY in localstorage   [\#240](https://github.com/gisaia/ARLAS-wui-builder/issues/240) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Bug after Log out then log in [\#239](https://github.com/gisaia/ARLAS-wui-builder/issues/239) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Persisted configuration doesn't set the visualisation set layers correctly [\#228](https://github.com/gisaia/ARLAS-wui-builder/issues/228) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Add docker env variable to set a default server URL [\#132](https://github.com/gisaia/ARLAS-wui-builder/issues/132) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Display 'date format' control only for date histograms [\#254](https://github.com/gisaia/ARLAS-wui-builder/pull/254) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix mat-option width [\#253](https://github.com/gisaia/ARLAS-wui-builder/pull/253) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add the possibility to parse multiple widgets in a group [\#252](https://github.com/gisaia/ARLAS-wui-builder/pull/252) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Homogenize css like in arlas-hub [\#251](https://github.com/gisaia/ARLAS-wui-builder/pull/251) ([sebbousquet](https://github.com/sebbousquet))
- Add docker env variable to set a default server URL [\#250](https://github.com/gisaia/ARLAS-wui-builder/pull/250) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Add env variable for settings file [\#249](https://github.com/gisaia/ARLAS-wui-builder/pull/249) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Fix path to translation files [\#245](https://github.com/gisaia/ARLAS-wui-builder/pull/245) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Store current configuration Id in the mainFormService [\#243](https://github.com/gisaia/ARLAS-wui-builder/pull/243) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Display landing page dialog after a callback redirection [\#242](https://github.com/gisaia/ARLAS-wui-builder/pull/242) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Use startupservice and persistence services of toolkit [\#238](https://github.com/gisaia/ARLAS-wui-builder/pull/238) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Upgrade arlas-persistence to 13.0.0-beta.2 [\#236](https://github.com/gisaia/ARLAS-wui-builder/pull/236) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- \[Visualisations sets\]:Transform sets to arrays in order to be stringi… [\#229](https://github.com/gisaia/ARLAS-wui-builder/pull/229) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v0.0.5](https://github.com/gisaia/ARLAS-wui-builder/tree/v0.0.5) (2020-07-24)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v0.0.4...v0.0.5)

**Closed issues:**

- Add a route to load a configuration from persistence [\#233](https://github.com/gisaia/ARLAS-wui-builder/issues/233) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Use new version of persistence-server [\#231](https://github.com/gisaia/ARLAS-wui-builder/issues/231)
- Persisted configuration has 'arlasWui' attribute instead of 'arlas-wui' [\#227](https://github.com/gisaia/ARLAS-wui-builder/issues/227) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Configuration stored in persistence should not contain 'extraConfigs' attribute [\#226](https://github.com/gisaia/ARLAS-wui-builder/issues/226) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]

**Merged pull requests:**

- Fix nginx conf and start script [\#235](https://github.com/gisaia/ARLAS-wui-builder/pull/235) ([sebbousquet](https://github.com/sebbousquet))
- Add route to load config directly from persistence [\#234](https://github.com/gisaia/ARLAS-wui-builder/pull/234) ([sebbousquet](https://github.com/sebbousquet))
- Use new version of persistence service [\#232](https://github.com/gisaia/ARLAS-wui-builder/pull/232) ([sebbousquet](https://github.com/sebbousquet))
- Fix/translation [\#225](https://github.com/gisaia/ARLAS-wui-builder/pull/225) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))

## [v0.0.4](https://github.com/gisaia/ARLAS-wui-builder/tree/v0.0.4) (2020-06-29)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v0.0.3...v0.0.4)

**Closed issues:**

- Translation are not take into account in sub modules [\#222](https://github.com/gisaia/ARLAS-wui-builder/issues/222) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Extra config part is missing in config.json [\#218](https://github.com/gisaia/ARLAS-wui-builder/issues/218) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Map contributor name is missing in configuration [\#217](https://github.com/gisaia/ARLAS-wui-builder/issues/217) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Add layer preview in map config [\#79](https://github.com/gisaia/ARLAS-wui-builder/issues/79) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Feature/translate extractor [\#221](https://github.com/gisaia/ARLAS-wui-builder/pull/221) ([sebbousquet](https://github.com/sebbousquet))
- Add extraconfig and map contributor name [\#220](https://github.com/gisaia/ARLAS-wui-builder/pull/220) ([mbarbet](https://github.com/mbarbet))
- Add global map preview [\#215](https://github.com/gisaia/ARLAS-wui-builder/pull/215) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)] ([mbarbet](https://github.com/mbarbet))

## [v0.0.3](https://github.com/gisaia/ARLAS-wui-builder/tree/v0.0.3) (2020-06-19)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/0.0.2...v0.0.3)

**Closed issues:**

- After a loading a config with a layer / manual color, elements are repeated in the colors list [\#211](https://github.com/gisaia/ARLAS-wui-builder/issues/211) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Some issues [\#204](https://github.com/gisaia/ARLAS-wui-builder/issues/204) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Default values reset doesn't work as expected on Timeline [\#189](https://github.com/gisaia/ARLAS-wui-builder/issues/189) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Make button toggler choice more intuive [\#187](https://github.com/gisaia/ARLAS-wui-builder/issues/187) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Layers list with arlas-wui legend [\#186](https://github.com/gisaia/ARLAS-wui-builder/issues/186) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- LOAD and SAVE unmanaged fields, keeping original values [\#160](https://github.com/gisaia/ARLAS-wui-builder/issues/160) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Create interface to import configuration from persistence  [\#151](https://github.com/gisaia/ARLAS-wui-builder/issues/151) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Edit tab name reload content and reset AB tab display [\#150](https://github.com/gisaia/ARLAS-wui-builder/issues/150) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Save imported files [\#147](https://github.com/gisaia/ARLAS-wui-builder/issues/147) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- SEARCH: search field should be `text` and autocomplete\_field should be `keyword` [\#146](https://github.com/gisaia/ARLAS-wui-builder/issues/146) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- SEARCH : the steps of autcomplete\_size slider should be of size 1 [\#145](https://github.com/gisaia/ARLAS-wui-builder/issues/145) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Weight of heatmap / normalized count [\#144](https://github.com/gisaia/ARLAS-wui-builder/issues/144) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Choosing collection when configuring a map layer [\#143](https://github.com/gisaia/ARLAS-wui-builder/issues/143) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Timeline config bugs [\#142](https://github.com/gisaia/ARLAS-wui-builder/issues/142) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Wrong field name given to PROPERTY\_SELECTOR\_SOURCE.manual [\#140](https://github.com/gisaia/ARLAS-wui-builder/issues/140) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- LOAD and SAVE using persistence [\#138](https://github.com/gisaia/ARLAS-wui-builder/issues/138) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Refacto forms with config-form-controls [\#137](https://github.com/gisaia/ARLAS-wui-builder/issues/137)
- Avoid 2 widgets to have the same name [\#128](https://github.com/gisaia/ARLAS-wui-builder/issues/128) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- After a reset, some errors are still displayed if an export was attempted [\#119](https://github.com/gisaia/ARLAS-wui-builder/issues/119) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Display  analytics preview : update problem [\#111](https://github.com/gisaia/ARLAS-wui-builder/issues/111) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Create Side module config page [\#109](https://github.com/gisaia/ARLAS-wui-builder/issues/109) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Add preview for each analytics components [\#108](https://github.com/gisaia/ARLAS-wui-builder/issues/108) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Created a dialog for setting up each ANALYTICS WIDGET [\#90](https://github.com/gisaia/ARLAS-wui-builder/issues/90) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Fix of map layer edition [\#87](https://github.com/gisaia/ARLAS-wui-builder/issues/87) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Some visual improvments [\#86](https://github.com/gisaia/ARLAS-wui-builder/issues/86) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Create LOOK'N FEEL config page [\#71](https://github.com/gisaia/ARLAS-wui-builder/issues/71) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Feature/improve docker [\#216](https://github.com/gisaia/ARLAS-wui-builder/pull/216) ([sebbousquet](https://github.com/sebbousquet))
- Fix palette édition [\#214](https://github.com/gisaia/ARLAS-wui-builder/pull/214) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix manual colors duplicated. [\#212](https://github.com/gisaia/ARLAS-wui-builder/pull/212) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add layer icon in layers list [\#210](https://github.com/gisaia/ARLAS-wui-builder/pull/210) ([sebbousquet](https://github.com/sebbousquet))
- Fix AnalyticBoardComponent errors in preview [\#209](https://github.com/gisaia/ARLAS-wui-builder/pull/209) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix Powerbar title isn't displayed in the preview. [\#208](https://github.com/gisaia/ARLAS-wui-builder/pull/208) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/timeline reset [\#207](https://github.com/gisaia/ARLAS-wui-builder/pull/207) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/map issues [\#206](https://github.com/gisaia/ARLAS-wui-builder/pull/206) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/layer preview [\#203](https://github.com/gisaia/ARLAS-wui-builder/pull/203) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/missing unmanaged fields [\#202](https://github.com/gisaia/ARLAS-wui-builder/pull/202) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate MAP GLOBAL unmanaged fields [\#201](https://github.com/gisaia/ARLAS-wui-builder/pull/201) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate RESULTLIST unmanaged fields [\#200](https://github.com/gisaia/ARLAS-wui-builder/pull/200) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate DONUT unmanaged fields [\#199](https://github.com/gisaia/ARLAS-wui-builder/pull/199) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate METRIC unmanaged fields [\#198](https://github.com/gisaia/ARLAS-wui-builder/pull/198) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate SWIMLANE unmanaged fields [\#197](https://github.com/gisaia/ARLAS-wui-builder/pull/197) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate HISTOGRAM unmanaged fields [\#196](https://github.com/gisaia/ARLAS-wui-builder/pull/196) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate TIMELINE unmanaged fields [\#195](https://github.com/gisaia/ARLAS-wui-builder/pull/195) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Propagate SEARCH unmanaged fields [\#194](https://github.com/gisaia/ARLAS-wui-builder/pull/194) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Reset all errors after a reset [\#193](https://github.com/gisaia/ARLAS-wui-builder/pull/193) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add an optionnal label field on provided color selector [\#192](https://github.com/gisaia/ARLAS-wui-builder/pull/192) ([sebbousquet](https://github.com/sebbousquet))
- Make ButtonToggler more intuitive [\#191](https://github.com/gisaia/ARLAS-wui-builder/pull/191) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/side modules [\#190](https://github.com/gisaia/ARLAS-wui-builder/pull/190) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Use array for color field and provided color in map layer [\#188](https://github.com/gisaia/ARLAS-wui-builder/pull/188) ([sebbousquet](https://github.com/sebbousquet))
- Fix timeline import & export [\#185](https://github.com/gisaia/ARLAS-wui-builder/pull/185) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Map-global with config-forms [\#183](https://github.com/gisaia/ARLAS-wui-builder/pull/183) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Timeline: Use config form tabs instead of custom ones [\#182](https://github.com/gisaia/ARLAS-wui-builder/pull/182) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Create analytics tabs with unique name. [\#181](https://github.com/gisaia/ARLAS-wui-builder/pull/181) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Make sure that an interval value is an integer [\#180](https://github.com/gisaia/ARLAS-wui-builder/pull/180) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Widgets with tabs instead of steps. [\#179](https://github.com/gisaia/ARLAS-wui-builder/pull/179) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Put a preview of a selected palette below the palette edition button [\#178](https://github.com/gisaia/ARLAS-wui-builder/pull/178) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix some swimlane issues [\#177](https://github.com/gisaia/ARLAS-wui-builder/pull/177) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/slidetoggle to buttons [\#176](https://github.com/gisaia/ARLAS-wui-builder/pull/176) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Avoid 2 tabs to have the same name. [\#175](https://github.com/gisaia/ARLAS-wui-builder/pull/175) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix analytics tabs focus after tab creation. [\#173](https://github.com/gisaia/ARLAS-wui-builder/pull/173) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add details to resultlist [\#172](https://github.com/gisaia/ARLAS-wui-builder/pull/172) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Update angular icon font [\#171](https://github.com/gisaia/ARLAS-wui-builder/pull/171) ([sebbousquet](https://github.com/sebbousquet))
- Create Resultlist widget [\#170](https://github.com/gisaia/ARLAS-wui-builder/pull/170) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix donut size on load [\#169](https://github.com/gisaia/ARLAS-wui-builder/pull/169) ([sebbousquet](https://github.com/sebbousquet))
- Move 'Add button' for analytics tabs [\#168](https://github.com/gisaia/ARLAS-wui-builder/pull/168) ([sebbousquet](https://github.com/sebbousquet))
- Fix import of spinner option in look and feel form [\#167](https://github.com/gisaia/ARLAS-wui-builder/pull/167) ([sebbousquet](https://github.com/sebbousquet))
- Fix search field & autocomplete field [\#166](https://github.com/gisaia/ARLAS-wui-builder/pull/166) ([sebbousquet](https://github.com/sebbousquet))
- Feature/analytics home [\#165](https://github.com/gisaia/ARLAS-wui-builder/pull/165) ([sebbousquet](https://github.com/sebbousquet))
- Move actions in drop down menu in presentation table [\#164](https://github.com/gisaia/ARLAS-wui-builder/pull/164) ([sebbousquet](https://github.com/sebbousquet))
- Create Look And Feel config page [\#163](https://github.com/gisaia/ARLAS-wui-builder/pull/163) ([sebbousquet](https://github.com/sebbousquet))
- Improve landing page [\#162](https://github.com/gisaia/ARLAS-wui-builder/pull/162) ([sebbousquet](https://github.com/sebbousquet))
- Add metric & powerbars widgets [\#161](https://github.com/gisaia/ARLAS-wui-builder/pull/161) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Separate Save and Export actions [\#159](https://github.com/gisaia/ARLAS-wui-builder/pull/159) ([sebbousquet](https://github.com/sebbousquet))
- Add a name to the configuration object in persistence [\#158](https://github.com/gisaia/ARLAS-wui-builder/pull/158) ([sebbousquet](https://github.com/sebbousquet))
- Add metric widget [\#157](https://github.com/gisaia/ARLAS-wui-builder/pull/157) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Load a conf and update it [\#156](https://github.com/gisaia/ARLAS-wui-builder/pull/156) ([sebbousquet](https://github.com/sebbousquet))
- Fix global validation [\#155](https://github.com/gisaia/ARLAS-wui-builder/pull/155) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add config management [\#154](https://github.com/gisaia/ARLAS-wui-builder/pull/154) ([sebbousquet](https://github.com/sebbousquet))
- Feature/refacto map layers [\#153](https://github.com/gisaia/ARLAS-wui-builder/pull/153) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/fix\#111 [\#152](https://github.com/gisaia/ARLAS-wui-builder/pull/152) ([sebbousquet](https://github.com/sebbousquet))
- Switch Timeline to config-form-elements [\#141](https://github.com/gisaia/ARLAS-wui-builder/pull/141) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Switch Search to config-form-elements [\#139](https://github.com/gisaia/ARLAS-wui-builder/pull/139) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/map config import [\#136](https://github.com/gisaia/ARLAS-wui-builder/pull/136) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/prepare map config import [\#135](https://github.com/gisaia/ARLAS-wui-builder/pull/135) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Update to 0.0.3 [\#134](https://github.com/gisaia/ARLAS-wui-builder/pull/134) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))

## [0.0.2](https://github.com/gisaia/ARLAS-wui-builder/tree/0.0.2) (2020-04-30)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/v0.0.1...0.0.2)

**Closed issues:**

- Define interpolated style in cluster mode  [\#116](https://github.com/gisaia/ARLAS-wui-builder/issues/116) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- After cancelling a new map layer edit, user is back to the global tab but it's not selected [\#107](https://github.com/gisaia/ARLAS-wui-builder/issues/107) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Use content of formGroup to display preview [\#106](https://github.com/gisaia/ARLAS-wui-builder/issues/106) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Add a hardcoded preview in analytics group  config [\#100](https://github.com/gisaia/ARLAS-wui-builder/issues/100) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Bug : checkUrl in landing page fonction not work if a default value is used for url [\#99](https://github.com/gisaia/ARLAS-wui-builder/issues/99) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Add creation of collaboration search Service and update config service [\#98](https://github.com/gisaia/ARLAS-wui-builder/issues/98)
- Remove useless  config.json file [\#97](https://github.com/gisaia/ARLAS-wui-builder/issues/97)
- Init an empty json for arlas configuration service in startup service [\#96](https://github.com/gisaia/ARLAS-wui-builder/issues/96)
- Remove init of arlas collaborative search service from startupService [\#95](https://github.com/gisaia/ARLAS-wui-builder/issues/95) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Put all material dependencies into the shared project [\#84](https://github.com/gisaia/ARLAS-wui-builder/issues/84) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
-  After resetting by clicking the banner, the collection doesn't appear anymore [\#77](https://github.com/gisaia/ARLAS-wui-builder/issues/77) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- In layer edition, normalization can be applied also to float and doubles [\#129](https://github.com/gisaia/ARLAS-wui-builder/issues/129) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- The URL pattern validator does not accept all urls [\#124](https://github.com/gisaia/ARLAS-wui-builder/issues/124) [[type:bug](https://github.com/gisaia/ARLAS-wui-builder/labels/type:bug)]
- Add LOAD of existing config file [\#104](https://github.com/gisaia/ARLAS-wui-builder/issues/104) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Plug arlas-persistance [\#78](https://github.com/gisaia/ARLAS-wui-builder/issues/78) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Create ANALYTICS config page [\#70](https://github.com/gisaia/ARLAS-wui-builder/issues/70) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Fix Map Preview [\#131](https://github.com/gisaia/ARLAS-wui-builder/pull/131) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Update url regex to allow IP+port/domain/localhost+port as server URL [\#130](https://github.com/gisaia/ARLAS-wui-builder/pull/130) ([sebbousquet](https://github.com/sebbousquet))
- Fix default values [\#125](https://github.com/gisaia/ARLAS-wui-builder/pull/125) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Feature/persistence [\#123](https://github.com/gisaia/ARLAS-wui-builder/pull/123) ([sebbousquet](https://github.com/sebbousquet))
- import LayerSourceConfig from arlas-web-contributors + generate source name [\#120](https://github.com/gisaia/ARLAS-wui-builder/pull/120) ([MohamedHamouGisaia](https://github.com/MohamedHamouGisaia))
- Feature/add map layer preview [\#117](https://github.com/gisaia/ARLAS-wui-builder/pull/117) ([mbarbet](https://github.com/mbarbet))
- Fix swimlane contrib export [\#114](https://github.com/gisaia/ARLAS-wui-builder/pull/114) ([mbarbet](https://github.com/mbarbet))
- Add a button to valid arlas url [\#113](https://github.com/gisaia/ARLAS-wui-builder/pull/113) ([mbarbet](https://github.com/mbarbet))
- Fix \#77 [\#112](https://github.com/gisaia/ARLAS-wui-builder/pull/112) ([mbarbet](https://github.com/mbarbet))
- Display analytic preview with form data [\#110](https://github.com/gisaia/ARLAS-wui-builder/pull/110) ([mbarbet](https://github.com/mbarbet))
- Feature/init preview [\#103](https://github.com/gisaia/ARLAS-wui-builder/pull/103) ([mbarbet](https://github.com/mbarbet))
- Feature/fix init collab service [\#101](https://github.com/gisaia/ARLAS-wui-builder/pull/101) ([mbarbet](https://github.com/mbarbet))
- Feature/use toolkit shared module [\#94](https://github.com/gisaia/ARLAS-wui-builder/pull/94) ([mbarbet](https://github.com/mbarbet))
- Release 0.0.2 [\#133](https://github.com/gisaia/ARLAS-wui-builder/pull/133) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/issue 116 [\#127](https://github.com/gisaia/ARLAS-wui-builder/pull/127) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/issue 87 [\#126](https://github.com/gisaia/ARLAS-wui-builder/pull/126) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/import timeline [\#122](https://github.com/gisaia/ARLAS-wui-builder/pull/122) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Import an existing config: search [\#121](https://github.com/gisaia/ARLAS-wui-builder/pull/121) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add Map component section in global form [\#118](https://github.com/gisaia/ARLAS-wui-builder/pull/118) ([sebbousquet](https://github.com/sebbousquet))
- Import an existing config file: widgets [\#115](https://github.com/gisaia/ARLAS-wui-builder/pull/115) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Turn widget builder services to state-less [\#105](https://github.com/gisaia/ARLAS-wui-builder/pull/105) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Export histogram & swimlane [\#102](https://github.com/gisaia/ARLAS-wui-builder/pull/102) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/shared imports [\#93](https://github.com/gisaia/ARLAS-wui-builder/pull/93) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Version/0.0.2 [\#92](https://github.com/gisaia/ARLAS-wui-builder/pull/92) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))

## [v0.0.1](https://github.com/gisaia/ARLAS-wui-builder/tree/v0.0.1) (2020-04-14)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/0.0.0...v0.0.1)

**Closed issues:**

- Create TIMELINE config page [\#6](https://github.com/gisaia/ARLAS-wui-builder/issues/6) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Create SEARCH config page [\#5](https://github.com/gisaia/ARLAS-wui-builder/issues/5) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Fix dockerfile and build option [\#89](https://github.com/gisaia/ARLAS-wui-builder/pull/89) ([sebbousquet](https://github.com/sebbousquet))
- Manage count step of interpolation [\#88](https://github.com/gisaia/ARLAS-wui-builder/pull/88) ([sebbousquet](https://github.com/sebbousquet))
- Feature/cluster export [\#85](https://github.com/gisaia/ARLAS-wui-builder/pull/85) ([sebbousquet](https://github.com/sebbousquet))
- Feature/cluster style [\#80](https://github.com/gisaia/ARLAS-wui-builder/pull/80) ([sebbousquet](https://github.com/sebbousquet))
- Feature/cluster layer [\#76](https://github.com/gisaia/ARLAS-wui-builder/pull/76) ([sebbousquet](https://github.com/sebbousquet))
- Fix edition of a layer [\#69](https://github.com/gisaia/ARLAS-wui-builder/pull/69) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Export timeline configuration. [\#68](https://github.com/gisaia/ARLAS-wui-builder/pull/68) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add "timeline" configuration. [\#67](https://github.com/gisaia/ARLAS-wui-builder/pull/67) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/search [\#66](https://github.com/gisaia/ARLAS-wui-builder/pull/66) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/edition missing values [\#65](https://github.com/gisaia/ARLAS-wui-builder/pull/65) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Export layer with mode feature-metric [\#64](https://github.com/gisaia/ARLAS-wui-builder/pull/64) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Custom "style" for layer feature-metric [\#63](https://github.com/gisaia/ARLAS-wui-builder/pull/63) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add edit-layer-mode-form as generic layer component. [\#62](https://github.com/gisaia/ARLAS-wui-builder/pull/62) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- "feature-metric" layer mode [\#59](https://github.com/gisaia/ARLAS-wui-builder/pull/59) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))

## [0.0.0](https://github.com/gisaia/ARLAS-wui-builder/tree/0.0.0) (2020-03-19)

[Full Changelog](https://github.com/gisaia/ARLAS-wui-builder/compare/316eabb2d72ec53f8efccf934d40d8919c3dc537...0.0.0)

**Closed issues:**

- Add translation  [\#29](https://github.com/gisaia/ARLAS-wui-builder/issues/29) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Theme application [\#7](https://github.com/gisaia/ARLAS-wui-builder/issues/7) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Create MAP config page [\#4](https://github.com/gisaia/ARLAS-wui-builder/issues/4) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Landing page [\#3](https://github.com/gisaia/ARLAS-wui-builder/issues/3) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Manage default values  [\#2](https://github.com/gisaia/ARLAS-wui-builder/issues/2) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]
- Create navigation menu [\#1](https://github.com/gisaia/ARLAS-wui-builder/issues/1) [[type:enhancement](https://github.com/gisaia/ARLAS-wui-builder/labels/type:enhancement)]

**Merged pull requests:**

- Organize imports and add missing translation [\#54](https://github.com/gisaia/ARLAS-wui-builder/pull/54) ([sebbousquet](https://github.com/sebbousquet))
- Use translation pipe everywhere [\#53](https://github.com/gisaia/ARLAS-wui-builder/pull/53) ([sebbousquet](https://github.com/sebbousquet))
- Fix app initilisation with translation [\#51](https://github.com/gisaia/ARLAS-wui-builder/pull/51) ([sebbousquet](https://github.com/sebbousquet))
- Add translate service [\#48](https://github.com/gisaia/ARLAS-wui-builder/pull/48) ([sebbousquet](https://github.com/sebbousquet))
- Use a custom startup service instead of toolkit one [\#45](https://github.com/gisaia/ARLAS-wui-builder/pull/45) ([sebbousquet](https://github.com/sebbousquet))
- Init map geometry fields with 'geometry\_path' value [\#42](https://github.com/gisaia/ARLAS-wui-builder/pull/42) ([sebbousquet](https://github.com/sebbousquet))
- Feature/connect arlas [\#33](https://github.com/gisaia/ARLAS-wui-builder/pull/33) ([sebbousquet](https://github.com/sebbousquet))
- Fix/after meeting [\#30](https://github.com/gisaia/ARLAS-wui-builder/pull/30) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- ConfigElement also transcludes description [\#27](https://github.com/gisaia/ARLAS-wui-builder/pull/27) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Init map layer field with default values from json file [\#26](https://github.com/gisaia/ARLAS-wui-builder/pull/26) ([sebbousquet](https://github.com/sebbousquet))
- Add map config with mode "features" [\#22](https://github.com/gisaia/ARLAS-wui-builder/pull/22) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Layer edition is sub-route of layers [\#20](https://github.com/gisaia/ARLAS-wui-builder/pull/20) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Initialize default values loader [\#16](https://github.com/gisaia/ARLAS-wui-builder/pull/16) ([sebbousquet](https://github.com/sebbousquet))
- Display start choice only at root path [\#13](https://github.com/gisaia/ARLAS-wui-builder/pull/13) ([sebbousquet](https://github.com/sebbousquet))
- Add travis file [\#12](https://github.com/gisaia/ARLAS-wui-builder/pull/12) ([sebbousquet](https://github.com/sebbousquet))
- Add landing page [\#11](https://github.com/gisaia/ARLAS-wui-builder/pull/11) ([sebbousquet](https://github.com/sebbousquet))
- Use dedicated modules for each part of app [\#10](https://github.com/gisaia/ARLAS-wui-builder/pull/10) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add Dockerfile [\#61](https://github.com/gisaia/ARLAS-wui-builder/pull/61) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix validation fails with default geometry type [\#60](https://github.com/gisaia/ARLAS-wui-builder/pull/60) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Optimize angular material imports [\#58](https://github.com/gisaia/ARLAS-wui-builder/pull/58) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add width & radius to map features mode [\#57](https://github.com/gisaia/ARLAS-wui-builder/pull/57) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Move color selection in dedicated component... [\#56](https://github.com/gisaia/ARLAS-wui-builder/pull/56) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Improve material import & Add tools to analyze bundle [\#55](https://github.com/gisaia/ARLAS-wui-builder/pull/55) ([sebbousquet](https://github.com/sebbousquet))
- Export to config.json & config.map.json [\#52](https://github.com/gisaia/ARLAS-wui-builder/pull/52) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Feature/global validation [\#50](https://github.com/gisaia/ARLAS-wui-builder/pull/50) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/visual improvements [\#47](https://github.com/gisaia/ARLAS-wui-builder/pull/47) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Improve color interpolation [\#46](https://github.com/gisaia/ARLAS-wui-builder/pull/46) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Better manual color management. [\#44](https://github.com/gisaia/ARLAS-wui-builder/pull/44) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Use default.json for setting default config params [\#43](https://github.com/gisaia/ARLAS-wui-builder/pull/43) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Set shortest path in TS imports. [\#41](https://github.com/gisaia/ARLAS-wui-builder/pull/41) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Get collection fields values from real server. [\#40](https://github.com/gisaia/ARLAS-wui-builder/pull/40) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Get collection fields name and values from real server. [\#39](https://github.com/gisaia/ARLAS-wui-builder/pull/39) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add choice of geometry field for each collection [\#38](https://github.com/gisaia/ARLAS-wui-builder/pull/38) ([sebbousquet](https://github.com/sebbousquet))
- Use defaults in starting conf [\#37](https://github.com/gisaia/ARLAS-wui-builder/pull/37) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Check licence in every file from the CI [\#36](https://github.com/gisaia/ARLAS-wui-builder/pull/36) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add  input `fullSize`  to config-element [\#35](https://github.com/gisaia/ARLAS-wui-builder/pull/35) ([sebbousquet](https://github.com/sebbousquet))
- Dynamic default values. [\#34](https://github.com/gisaia/ARLAS-wui-builder/pull/34) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Layer color interpolation [\#32](https://github.com/gisaia/ARLAS-wui-builder/pull/32) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add generated + color source [\#31](https://github.com/gisaia/ARLAS-wui-builder/pull/31) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Better sliders displaying [\#28](https://github.com/gisaia/ARLAS-wui-builder/pull/28) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- alert-on-change the color source [\#25](https://github.com/gisaia/ARLAS-wui-builder/pull/25) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add ngx-logger [\#24](https://github.com/gisaia/ARLAS-wui-builder/pull/24) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Map config features with visibility & style [\#23](https://github.com/gisaia/ARLAS-wui-builder/pull/23) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Confirm exit once a layer is being edited [\#21](https://github.com/gisaia/ARLAS-wui-builder/pull/21) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add helper methods in MainService [\#19](https://github.com/gisaia/ARLAS-wui-builder/pull/19) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Global property to set snackbar timeout [\#18](https://github.com/gisaia/ARLAS-wui-builder/pull/18) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Fix/move shared module [\#17](https://github.com/gisaia/ARLAS-wui-builder/pull/17) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Add some unit tests of components. [\#15](https://github.com/gisaia/ARLAS-wui-builder/pull/15) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Basic Map configuration page [\#14](https://github.com/gisaia/ARLAS-wui-builder/pull/14) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Set default fonts + custom theme [\#9](https://github.com/gisaia/ARLAS-wui-builder/pull/9) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))
- Create navigation menu [\#8](https://github.com/gisaia/ARLAS-wui-builder/pull/8) ([laurent-thiebaud-gisaia](https://github.com/laurent-thiebaud-gisaia))



\* *This Changelog was automatically generated by [github_changelog_generator](https://github.com/github-changelog-generator/github-changelog-generator)*
